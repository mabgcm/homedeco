...
function mergeStoreProduct(seedProduct, liveProduct, sourceSyncedAt) {
  ...
  const seedImageIsReal = Boolean(
    seedProduct.image && !String(seedProduct.image).startsWith("data:image")
  );
  const image = seedImageIsReal ? seedProduct.image : (liveMapped?.image || seedProduct.image);
